...
def scale_process(model, image, classes, crop_h, crop_w, h, w, mean, std=None, stride_rate=2 / 3):
    ...
    for index_h in range(0, grid_h):
        for index_w in range(0, grid_w):
            ...
            tmp_pred, tmp_infer_time = net_process(model, image_crop, mean, std)
            prediction_crop[s_h:e_h, s_w:e_w, :] += tmp_pred
            total_infer_time += tmp_infer_time
    prediction_crop /= np.expand_dims(count_crop, 2)
    prediction_crop = prediction_crop[pad_h_half:pad_h_half + ori_h, pad_w_half:pad_w_half + ori_w]
    prediction = cv2.resize(prediction_crop, (w, h), interpolation=cv2.INTER_LINEAR)
    return prediction, total_infer_time
...
def test(test_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, scales, gray_folder,
    ...
    for i, (input, _) in enumerate(test_loader):
        ...
        for scale in scales:
            ...
            tmp_pred, tmp_infer_time = scale_process(model, image_scale, classes, crop_h, crop_w, h, w, mean, std)
            prediction += tmp_pred
        ...
        if ((i + 1) % 10 == 0) or (i + 1 == len(test_loader)):
            logger.info('Test: [{}/{}] '
                        'Data {data_time.val:.3f} ({data_time.avg:.3f}) '
                         ...
                         'Infer {infer_time.val:.3f} ({infer_time.avg:.3f}) '.format(
                i + 1, len(test_loader), data_time=data_time, batch_time=batch_time, infer_time=infer_time)
            )
...
def evaluate(test_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w,
             scales, gray_folder, color_folder, colors, names):
    ...
    for i, (input, _) in enumerate(test_loader):
        ...
        if (i % 50 == 0) or (i + 1 == len(test_loader)):
            logging.info('Test: [{}/{}] '
                         'Data {data_time.val:.3f} ({data_time.avg:.3f}) '
                         'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) '
                         'Infer {infer_time.val:.3f} ({infer_time.avg:.3f}) '.format(
