...
def train_pruned_model(args, train_loader, model, optimizer, epoch, criterion):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    main_loss_meter = AverageMeter()
    loss_meter = AverageMeter()
    ...
    max_iter = args.finetune_epochs * len(train_loader)
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        current_iter = epoch * len(train_loader) + i + 1
        current_lr = poly_learning_rate(args.finetune_lr, current_iter, max_iter, power=args.power)
        # for index in range(0, args.index_split):
        #     optimizer.param_groups[index]['lr'] = current_lr
        # for index in range(args.index_split, len(optimizer.param_groups)):
        #     optimizer.param_groups[index]['lr'] = current_lr * 10
        ...
        if (i + 1) % args.print_freq == 0:
            logging.info('Epoch: [{}/{}][{}/{}] '
                         ...
                         'MainLoss {main_loss_meter.val:.4f} '
                         'Loss {loss_meter.val:.4f} '
                         'Accuracy {accuracy:.4f}.'.format(epoch + 1, args.finetune_epochs, i + 1, len(train_loader),
                                                           batch_time=batch_time,
                                                           data_time=data_time,
                                                           remain_time=remain_time,
                                                           main_loss_meter=main_loss_meter,
    ...
    mAcc = np.mean(accuracy_class)
    allAcc = sum(intersection_meter.sum) / (sum(target_meter.sum) + 1e-10)
    logging.info('Train result at epoch [{}/{}]: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'
                 .format(epoch + 1, args.finetune_epochs, mIoU, mAcc, allAcc))
    return main_loss_meter.avg, mIoU, mAcc, allAcc
