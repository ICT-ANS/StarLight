...
from semseg.util.util import AverageMeter, intersectionAndUnion, check_makedirs, colorize

cv2.ocl.setUseOpenCL(False)
infer_time_global = AverageMeter()
...
def main():
    ...
    # colors = np.loadtxt(os.path.join(base_dir, args.colors_path)).astype('uint8')
    # names = [line.rstrip('\n') for line in open(os.path.join(base_dir, args.names_path))]
...
def net_process(model, image, mean, std=None, is_trt=False, flip=True):
    ...
    with torch.no_grad():
        if is_trt:
            trt_input = input
            res, infer_time = model.inference(trt_input)
            output = res.reshape([-1, 19, 713, 713])
        else:
            start = time.time()
            output = model(input)
            infer_time = time.time() - start
    global infer_time_global
    infer_time_global.update(infer_time, n=input.shape[0])
...
def scale_process(model, image, classes, crop_h, crop_w, h, w, mean, std=None, is_trt=False, stride_rate=2/3):
    ...
    for index_h in range(0, grid_h):
        for index_w in range(0, grid_w):
            ...
            prediction_crop[s_h:e_h, s_w:e_w, :] += net_process(model, image_crop, mean, std, is_trt)
...
def evaluate_quan(test_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w,
             scales, gray_folder, color_folder, colors, names, is_trt):
    ...
    for i, (input, _) in enumerate(test_loader):
        ...
        for scale in scales:
            ...
            if h > w:
                ...
                new_h = round(long_size/float(w)*h)
            image_scale = cv2.resize(image, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
            prediction += scale_process(model, image_scale, classes, crop_h, crop_w, h, w, mean, std, is_trt)
        prediction /= len(scales)
        prediction = np.argmax(prediction, axis=2)
        batch_time.update(time.time() - end)
        end = time.time()
    ...
    mIoU, mAcc, allAcc = cal_acc(data_list, gray_folder, classes, names)

    global infer_time_global
    logging.info('infer_time_per_img: %.3f \t FPS: %.3f' %
                 (infer_time_global.avg, 1 / infer_time_global.avg))
...
def cal_acc(data_list, pred_folder, classes, names):
    ...
    for i, (image_path, target_path) in enumerate(data_list):
        ...
        if i % 50 == 0:
            logging.info('Evaluating {0}/{1} on image {2}, accuracy {3:.4f}.'
    ...
    print('\n')
    logging.info('Eval result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(mIoU, mAcc, allAcc))
    for i in range(classes):
        logging.info('Class_{} result: iou/accuracy {:.4f}/{:.4f}, name: {}.'.
