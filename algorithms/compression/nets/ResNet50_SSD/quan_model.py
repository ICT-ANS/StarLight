...
class SSD(nn.Module):
    ...
    def inference(self, x):
        ...
        for start_idx in range(0, x.shape[0], self.batchsize):
            ...
            t1 = time.time()
            outs, infer_time = self.do_inference_v2(self.engine.context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream)
            # elapsed_time += time.time() - t1
            elapsed_time += infer_time
            shape = outs[0].shape[0]
            cls = outs[0][0:int(shape * effective_batch_size / self.batchsize)].reshape(effective_batch_size, 8732, 4)
            result_cls.append(cls.copy())
            shape = outs[1].shape[0]
            loc = outs[1][0:int(shape * effective_batch_size / self.batchsize)].reshape(effective_batch_size, 8732, 21)
