...
def arg_parse():
    ...
    parser.add_argument('--save_folder', default='./SSD_Pytorch/weights/', help='Location to save checkpoint models')
...
def save_checkpoint(net, epoch, size, optimizer):
    ...
    torch.save({'epoch': epoch, 'size': size, 'batch_size': cfg.TRAIN.BATCH_SIZE, 'model': net.state_dict(), 'optimizer': optimizer.state_dict()}, save_name)
