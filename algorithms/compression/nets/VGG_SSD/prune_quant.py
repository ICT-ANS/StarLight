...
def arg_parse():
    ...
    parser.add_argument('--save_dir', default='./prune_quant', help='The directory used to save the trained models', type=str)
...
def main():
    """
    The main function to prune and quant the network SSD,
    it contains that prune the network,
    quant the network,
    and eval the prune and quant network performance
    """    
    global args
    args = arg_parse()
    ...
    model.load_state_dict(torch.load(os.path.join(args.save_dir, 'model_pruned_finetune_{}.pth'.format(args.finetune_epochs))))
