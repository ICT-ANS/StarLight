...
if str(config_dir) not in sys.path: sys.path.insert(0, str(config_dir))
print(lib_dir)
print(sys.path)
...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger):
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(xloader):
        ...
        arch_targets = arch_targets.to(device)
...
def main(xargs):
    ...
    xargs.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    ...
    search_loader, _, valid_loader = get_nas_search_loaders(
        ...
        os.path.join("/".join(xargs.config_path.split('/')[:-2]), 'nas-benchmark'),
        config.batch_size, xargs.workers, xargs.debug
    )
    ...
    network, criterion = torch.nn.DataParallel(search_model).to(xargs.device), criterion.to(xargs.device)
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        logger.log(time_string() + ' ' + 'train_acc {:}'.format(search_w_top1))
        logger.log(time_string() + ' ' + 'train_loss {:}'.format(search_w_loss))
        logger.log(time_string() + ' ' + 'valid_acc {:}'.format(valid_a_top1))
        logger.log(time_string() + ' ' + 'valid_loss {:}'.format(valid_a_loss))
        ...
        logger.log(time_string() + ' ' + 'genotype = {:}'.format(genotypes[epoch]))
        # save checkpoint
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', type=str, help='Debug or False')
