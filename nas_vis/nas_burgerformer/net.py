...
from nas_vis.nas_burgerformer.block import Block
...
class UnifiedArch(nn.Module):
    ...
    """
    def __init__(self, net_config, H=None, W=None, mlp_ratios=None, downsamples=None, num_heads=None, act_layer=nn.GELU, norm_layer=GroupNorm, num_classes=1000, in_patch_size=7, in_stride=4, in_pad=2, down_patch_size=3, down_stride=2, down_pad=1, drop_rate=0., drop_path_rate=0., use_layer_scale=True, layer_scale_init_value=1e-5, fork_feat=False, init_cfg=None, pretrained=None, **kwargs):
        ...
        for i in range(len(layers)):
            stage = basic_blocks(net_config['stage-{}'.format(i + 1)], embed_dims[i], H[i], W[i], i, layers, num_heads=num_heads[i], mlp_ratio=mlp_ratios[i], act_layer=act_layer, drop_rate=drop_rate, drop_path_rate=drop_path_rate, use_layer_scale=use_layer_scale, layer_scale_init_value=layer_scale_init_value, expand_ratio=expand_ratios[i])
    ...
    def forward(self, x, cal_flops=False):
        if cal_flops:
            x, flops_embed = self.forward_embeddings(x, cal_flops)
            x, flops_token = self.forward_tokens(x, cal_flops)
            if self.fork_feat:
                return x
            x = self.norm(x)
            cls_out = self.head(x.mean([-2, -1]))
            flops_norm_head = self.flops(x.size(), cls_out.size())
            # for image classification
            print(flops_embed, flops_token, flops_norm_head)
            return cls_out, flops_embed + flops_token + flops_norm_head
    ...
    def flops(self, input_size, output_size):
        flops = 0
        flops += 2 * input_size[1] * input_size[2] * input_size[3]  # self.norm
        flops += self.head.in_features * self.head.out_features  # self.head
        return flops


@register_model
def unifiedarch(pretrained=False, **kwargs):
    """
    ...
    model.default_cfg = default_cfgs['unifiedarch']
    return model
...
if __name__ == "__main__":
    ...
    supernet = unifiedarch(net_config=poolformer_s12_real_2).cuda()
    num_params = 0
    ...
    macs, params = profile(supernet, inputs=(x, ), custom_ops={nn.GELU: count_gelu})
    print(macs / 1e9 / 2, params / 1e6)
