...
def from_burgerformer_read_log(method, log_name, key_words):
    """
    This function reads information from the log.
    ...
    # print(log_name)
    with open(log_name) as f:
        if method == 'BurgerFormer':
            for line in f:
                if 'Iter [' in line:
                    line = line.strip().split()
                    info['acc'].append(float(line[7][:-1]))
                    info['flops'].append(float(line[10][:-2]))
                    info['params'].append(float(line[13][:-1]))
